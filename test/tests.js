...
function generateTests(modifiedPaths) {
  ...
  directories.forEach(function (dirName) {
    // exceptions
    if (dirName === '.git' || dirName === 'node_modules' || dirName === '.idea' || dirName === 'powershell' || dirName === 'test') {
      return;
    }
      ...
      tests.forEach(function (test) {
        it(test.args[0] + ' & ' + test.args[1] + ' should be valid', function () {
          ...
          return validateTemplate.apply(null, test.args)
            ...
            .catch(function (err) {
              var errorString = 'Template Validation Failed. Try deploying your template with the commands:\n';
